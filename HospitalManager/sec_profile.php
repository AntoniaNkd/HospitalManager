...
<head>
    ...
    $query = "UPDATE secratary SET username='$username', email='$email', phone='$phone',password='$password' WHERE id='$id'";
    $data = mysqli_query($db, $query);
    }
    $query1 = "SELECT * FROM secratary";
    $data1 = mysqli_query($db, $query1);
    while ($row = mysqli_fetch_array($data1)) {
    echo "<div class= 'btn' style = 'width: 50px;'><a href='sec_profile.php?update={$row['id']}'>{$row['username']}</a></div>";
    echo "<br />";
    }
    ?>
    <?php
    ...
    while ($row = mysqli_fetch_array($data2)) {
        echo "<b><a href='sec_profile.php?update={$row['id']}'>{$row['username']}</a></b>";
        echo "<br />";
    }
    ?>
